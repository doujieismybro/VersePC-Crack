...
var ANNOUNCEMENT_CONTENT = {
    ...
    body: `
        ...
        <div class="announcement-footer">
            <p style="text-align:right;margin-top:20px;font-weight:bold;">SeaLantern<br>2026 年 6 月</p>
        </div>
    `
};
...
async function updateGameStatus() {
    try {
        ...
        if (status.running) {
            ...
            statusText.textContent = 'SeaLanternCrack所有付费功能解锁，激活码随便输入';
...
// 麦香主题激活框
function openMaixiangActivateModal() {
    const modal = document.getElementById('maixiang-activate-modal');
    if (!modal) return;
    modal.style.display = 'flex';
    const input = document.getElementById('maixiang-activate-input');
    if (input) { input.value = ''; input.focus(); }
    const err = document.getElementById('maixiang-activate-error');
    if (err) err.style.display = 'none';
}

function closeMaixiangActivateModal() {
    const modal = document.getElementById('maixiang-activate-modal');
    if (modal) modal.style.display = 'none';
}

async function submitMaixiangInlineActivate() {
    const input = document.getElementById('maixiang-inline-input');
    const errEl = document.getElementById('maixiang-inline-error');
    const submitBtn = document.getElementById('maixiang-inline-submit');
    if (!input) return;
    ...
    try {
        ...
        if (result && result.success) {
            _maixiangActivated = true;
            const inlineBox = document.getElementById('maixiang-inline-activate');
...
async function selectWallpaper(element) {
    ...
    // 麦香主题需要 VT- 激活码解锁，未激活时显示内联激活框，不切换主题
    if (mode === 'auroraVideo') {
        if (!_maixiangActivated) {
            const inlineBox = document.getElementById('maixiang-inline-activate');
            if (inlineBox) {
                inlineBox.style.display = 'block';
                const inp = document.getElementById('maixiang-inline-input');
                if (inp) { inp.value = ''; inp.focus(); }
                const err = document.getElementById('maixiang-inline-error');
                if (err) err.style.display = 'none';
            }
            return;
        }
...
function renderSponsors(filter) {
    ...
    if (filtered.length === 0) {
        container.innerHTML = '<span class="sponsor-empty">' + (keyword ? '海晶灯crack，乐子豆杰' : '海晶灯crack，乐子豆杰') + '</span>';
        return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadMachineId();
    updateActivationStatus();
    setTimeout(refreshMaixiangLock, 800);
