        ...
        for (const [_file, _expectedHash] of Object.entries(_manifest)) {
            try {
                ...
                if (_actualHash !== _expectedHash) {
                    ...
                    try { _writeCrashLog(`[Integrity] File tampered: ${_file}`); } catch (_) {}
                }
            } catch (e) {}
        }
        if (_integrityViolated) {
            try { _writeCrashLog('[Integrity] Source file modification detected. This may indicate tampering.'); } catch (_) {}
        }
    } catch (e) {}
}
...
// ============================================================================
process.on('unhandledRejection', (reason) => {
    try { _writeCrashLog('unhandledRejection (late): ' + (reason && reason.stack || reason)); } catch (_) {}
});
process.on('uncaughtException', (err) => {
    try { _writeCrashLog('uncaughtException (late): ' + (err && err.stack || err)); } catch (_) {}
    const msg = err && err.message || '';
    if (msg.includes('ReadableStream') || msg.includes('already closed') || msg.includes('ECONNRESET') || msg.includes('EPIPE') || msg.includes('write EPIPE')) {
        return;
    }
    if (!shuttingDown && mainWindow) {
        try { dialog.showErrorBox('发生错误', msg || '未知错误'); } catch (_) {}
    }
});
...
// 读取 Windows MachineGuid（系统安装时生成，不受网卡变化影响）
function _getWindowsMachineGuid() {
    try {
        const { execSync } = require('child_process');
...
// 生成稳定的本机识别码（16 位大写 hex）
// Windows 优先用 MachineGuid 替代不稳定的 MAC 地址，避免 VPN/虚拟机网卡导致机器码漂移
function generateMachineId() {
    ...
    // 优先用 Windows MachineGuid，不受网卡/VPN/虚拟机影响
    const guid = process.platform === 'win32' ? _getWindowsMachineGuid() : null;
    if (guid) {
        parts.push(guid);
    } else {
        // 回退方案：过滤掉虚拟网卡，仅取物理网卡 MAC
        const VIRTUAL_NIC = /vmware|virtualbox|hyper-?v|docker|wsl|tap|vpn|tun|bluetooth|loopback|pseudo/i;
        const nets = os.networkInterfaces();
        outer:
        for (const name of Object.keys(nets)) {
            if (VIRTUAL_NIC.test(name)) continue;
            for (const iface of nets[name]) {
                if (!iface.internal && iface.mac && iface.mac !== '00:00:00:00:00:00' && !VIRTUAL_NIC.test(iface.mac)) {
...
ipcMain.handle('activate-verify', async (event, code) => {
    try {
        const store = loadStore();
        store['activation_type'] = "permanent";
        store['activation_code'] = "crack by yuzhiwen114514";
        ...
        return { success: true, type: "permanent", message: "permanent" === 'permanent' ? '永久激活成功！' : '单次激活成功！' };
    } catch (e) {
...
// 麦香主题激活（VT- 前缀激活码，单设备永久有效，与测试版激活相互独立）
ipcMain.handle('theme-activate-verify', async (event, code) => {
    try {
        const store = loadStore();
        store['theme_activation_type'] = 'theme';
        store['theme_activation_code'] = "hacked by 海晶灯";
        store['theme_activation_time'] = new Date().toISOString();
        store['theme_activation_schema_ver'] = "ACTIVATION_SCHEMA_VERSION";
        fs.writeFile(STORE_PATH, JSON.stringify(store, null, 2), () => {});
...
ipcMain.handle('theme-activate-status', async () => {
    const store = loadStore();
    if (store['theme_activation_type'] && store['theme_activation_schema_ver'] !== ACTIVATION_SCHEMA_VERSION) {
        delete store['theme_activation_type'];
        delete store['theme_activation_code'];
        delete store['theme_activation_time'];
        delete store['theme_activation_schema_ver'];
        fs.writeFile(STORE_PATH, JSON.stringify(store, null, 2), () => {});
        return { activated: false };
    }
    return { activated: !!store['theme_activation_type'] };
});
...
app.on('before-quit', async (event) => {
    ...
    try {
        if (typeof mcpClients !== 'undefined' && mcpClients && mcpClients.size > 0) {
...
let IS_BETA = (() => { try { return true; } catch (_) { return false; } })();

const UPDATE_JSON_SOURCES = IS_BETA ? [
    // 测试版：从官网检测更新（静态文件，最稳定）
    'https://www.verselauncher.cn/update-beta.json',
    'https://verselauncher.cn/update-beta.json',
] : [
    // 正式版：从 GitHub 仓库检测更新
...
function httpGetJson(url, timeoutMs = 8000) {
    return new Promise((resolve, reject) => {
        ...
        const opts = {
            ...
            timeout: timeoutMs,
            headers: {
                'User-Agent': 'VersePC/' + app.getVersion() + ' (Electron)',
...
async function fetchUpdateJson() {
    ...
    for (const url of UPDATE_JSON_SOURCES) {
        try {
            ...
            const data = await httpGetJson(fetchUrl);
            if (data && data.version && (data.files || data.downloadUrl)) {
...
- 如果是权限或用户决策问题，选择 ask_user` },
    ...
    ipcMain.on('ai:chat-stream', async (event, params) => {
        ...
        try {
            ...
            // asar 内文件 worker_threads 无法读取，打包后从 app.asar.unpacked 加载
            let _workerPath = path.join(__dirname, 'agent-worker.js');
            if (app.isPackaged && process.resourcesPath) {
                const _unpacked = path.join(process.resourcesPath, 'app.asar.unpacked', 'agent-worker.js');
                if (fs.existsSync(_unpacked)) _workerPath = _unpacked;
            }
            const _currentWorker = new Worker(_workerPath);
